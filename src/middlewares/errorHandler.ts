...
export function errorHandler(
  ...
  if (err instanceof AppError) {
    res.status(err.statusCode).json(
      apiError(err.message, isDev ? { stack: err.stack } : undefined),
    );
    return;
  }
  ...
  if (err instanceof Prisma.PrismaClientKnownRequestError) {
    switch (err.code) {
      ...
      case 'P2002': 
        res.status(409).json(
          apiError('Data already exists (unique constraint violated)', isDev ? err.meta : undefined),
        );
        return;
      case 'P2003': 
      ...
      default:
        res.status(400).json(
          apiError('Database request error', isDev ? { code: err.code, meta: err.meta } : undefined),
        );
        return;
    }
  }
  ...
  if (err instanceof ZodError) {
    res.status(422).json(
      apiError('Validation failed', err.flatten().fieldErrors),
    );
    return;
  }
